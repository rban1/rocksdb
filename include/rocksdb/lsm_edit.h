...
struct LsmEditFile {
  ...
  // newer, matching the order L0 files are consulted on a read.
  int level = 0;

  // Whole-file checksum of `path` and the name of the function that produced
  // it. Both empty means no checksum was supplied; see
  // LsmEditOptions::verify_file_checksum for how they are used.
  std::string checksum;
